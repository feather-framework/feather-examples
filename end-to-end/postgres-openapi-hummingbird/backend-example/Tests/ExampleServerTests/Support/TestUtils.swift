...
func makeRunner() async throws -> SpecRunnerHummingbird {
...
func runSpec(
    using runner: SpecRunnerHummingbird,
    ...
    let components = try await buildApplicationComponents(reader: reader)
    let freshRunner = SpecRunnerHummingbird(
        app: components.app,
        testingSetup: .router
    )
